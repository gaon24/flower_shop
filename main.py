...
flowers = [
    {"id": 1, "name": "Альстромерия", "price": 150, "color": "белые", 
     "description": "Нежные цветы символизируют дружбу и долголетие", 
     "image_url": "/static/photo/Альстромерия.jpg"},
]
...
def get_recommendations(flower_id):
    # Простая рекомендация: другие цветы из той же ценовой категории
    current = next((f for f in flowers if f['id'] == flower_id), None)
    if current:
        return [f for f in flowers if 
                f['id'] != flower_id and 
                abs(f['price'] - current['price']) <= 50][:3]
    return []

promotions = {
    "8_marta": {"discount": 15, "message": "🌷 К 8 марта скидка 15%!"},
    "14_feb": {"discount": 10, "message": "💕 День всех влюбленных - скидка 10%"},
    "new_year": {"discount": 20, "message": "🎄 Новогодние скидки до 20%!"}
}